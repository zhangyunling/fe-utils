...
export function hexToRgb (hex: string): string {
  ...
  // 以 # 号开头
  if (_hex.indexOf('#') !== 0) {
    throw new TypeError(`hexToRgb传入的参数格式不正确，必须要以#开头：${_hex}`);
  }
  ...
  const r = parseInt('0x' + _hex.slice(0, 2), 16);
  const g = parseInt('0x' + _hex.slice(2, 4), 16);
  const b = parseInt('0x' + _hex.slice(4, 6), 16);
  let a = 1;
